...
class TestXMLRE (unittest.TestCase):
    ...
    def testWildcardEscape (self):
        ...
        self.assertEqual(charset, str.WildcardEsc)
        self.assertEqual(position, 1)

    def testSingleCharEscapes (self):
        # 17 chars recognized as escapes
        self.assertEqual(len(str.SingleCharEsc), 17)
    ...
    def testMultiCharEscapes (self):
        ...
        self.assertEqual(len(str.MultiCharEsc), 10)
    ...
    def testMatchCharProperty (self):
        ...
        self.assertEqual(charset, str.PropertyMap['L'])
        text = "\p{IsCyrillic}"
        (charset, position) = xmlre._MatchCharClassEsc(text, 0)
        self.assertEqual(position, len(text))
        self.assertEqual(charset, str.BlockMap['Cyrillic'])
    ...
    def testCharProperty (self):
        ...
        self.assertEqual(position, len(text))
        self.assertEqual(charset, str.PropertyMap['N'])
        text = r'\P{N}'
        (charset, position) = xmlre.MaybeMatchCharacterClass(text, 0)
        self.assertEqual(position, len(text))
        self.assertEqual(charset.negate(), str.PropertyMap['N'])
        text = r'\p{Sm}'
        (charset, position) = xmlre.MaybeMatchCharacterClass(text, 0)
        self.assertEqual(position, len(text))
        self.assertEqual(charset, str.PropertyMap['Sm'])

    def testCharBlock (self):
        text = r'\p{IsArrows}'
        (charset, position) = xmlre.MaybeMatchCharacterClass(text, 0)
        self.assertEqual(position, len(text))
        self.assertEqual(charset, str.BlockMap['Arrows'])
        text = r'\P{IsArrows}'
        (charset, position) = xmlre.MaybeMatchCharacterClass(text, 0)
        self.assertEqual(position, len(text))
        self.assertEqual(charset.negate(), str.BlockMap['Arrows'])
    ...
    def testCharOrSCE (self):
        ...
        self.assertEqual(2, position)
        self.assertEqual(str.CodePointSet("\t"), charset)

    def testMatchPosCharGroup (self):
        text = 'A]'
        (charset, has_sub, position) = xmlre._MatchPosCharGroup(text, 0)
        self.assertEqual(position, 1)
        self.assertEqual(charset, str.CodePointSet(ord('A')))
        text = r'\n]'
        (charset, has_sub, position) = xmlre._MatchPosCharGroup(text, 0)
        self.assertEqual(position, 2)
        self.assertEqual(charset, str.CodePointSet(10))
        text = r'-]'
        (charset, has_sub, position) = xmlre._MatchPosCharGroup(text, 0)
        self.assertEqual(position, 1)
        self.assertEqual(charset, str.CodePointSet(ord('-')))
        text = 'A-Z]'
        (charset, has_sub, position) = xmlre._MatchPosCharGroup(text, 0)
        self.assertEqual(position, 3)
        self.assertEqual(charset, str.CodePointSet((ord('A'), ord('Z'))))
        text = r'\t-\r]'
        (charset, has_sub, position) = xmlre._MatchPosCharGroup(text, 0)
        self.assertEqual(position, 5)
        self.assertEqual(charset, str.CodePointSet((9, 13)))
        text = r'\t-A]'
        (charset, has_sub, position) = xmlre._MatchPosCharGroup(text, 0)
        self.assertEqual(position, 4)
        self.assertEqual(charset, str.CodePointSet((9, ord('A'))))
        text = r'Z-\]]'
        (charset, has_sub, position) = xmlre._MatchPosCharGroup(text, 0)
        self.assertEqual(position, 4)
        self.assertEqual(charset, str.CodePointSet((ord('Z'), ord(']'))))
        text = 'Z-A]'
    ...
    def testMatchCharClassExpr (self):
        ...
        first_five = str.CodePointSet( (ord('A'), ord('E')) )
        text = r'[ABCDE]'
        ...
        text = r'[A-Z-[GHI]]'
        expected = str.CodePointSet( (ord('A'), ord('Z')) )
        ...
        text = r'[\p{L}-[\p{Lo}]]'
        (charset, position) = xmlre._MatchCharClassExpr(text, 0)
        expected = str.CodePointSet(str.PropertyMap['L'])
        expected.subtract(str.PropertyMap['Lo'])
        self.assertEqual(position, len(text))
        self.assertEqual(charset, expected)
