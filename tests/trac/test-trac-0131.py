# Coding declaration for unicode strings
# -*- coding: utf-8 -*-
import logging
if __name__ == '__main__':
    logging.basicConfig()
_log = logging.getLogger(__name__)
# See also:
# http://www.evanjones.ca/python-utf8.html
# http://bytes.com/topic/python/answers/41153-xml-unicode-what-am-i-doing-wrong

import pyxb_123.binding.generate
import pyxb_123.binding.datatypes as xs
import pyxb_123.binding.basis
import pyxb_123.utils.domutils
import xml.sax
import pyxb_123.utils.types_ as types_
import io

import os.path
xsd='''<?xml version="1.0" encoding="UTF-8"?>
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
    <xs:element name="foo" type="xs:string"/>
    <xs:element name="bar">
        <xs:complexType>
            <xs:sequence>
                <xs:element name="e" type="xs:string" minOccurs="0"/>
            </xs:sequence>
            <xs:attribute name="a" type="xs:string"/>
        </xs:complexType>
    </xs:element>
</xs:schema>'''

code = pyxb_123.binding.generate.GeneratePython(schema_text=xsd)
#open('code.py', 'w').write(code)

rv = compile(code, 'test', 'exec')
eval(rv)

from pyxb_123.exceptions_ import *

import unittest

class TestTrac0131 (unittest.TestCase):
    # Unicode string, UTF-8 encoding (per declaration at script top)
    strt = u'Sign of Leser-Trélat'
    strd = strt.encode('utf-8')
    base_xmlt = u'<bar><e>' + strt + u'</e></bar>'
    declared_xmlt = u'<?xml version="1.0" encoding="UTF-8"?>' + base_xmlt

    def setUp (self):
        self.__xmlStyle = pyxb_123._XMLStyle

    def tearDown (self):
        pyxb_123._SetXMLStyle(self.__xmlStyle)

    def testRepresentation (self):
        self.assertEqual(self.strd, 'Sign of Leser-Tr\xc3\xa9lat') #!python3!
#python3:        self.assertEqual(self.strd, b'Sign of Leser-Tr\xc3\xa9lat')

    def testBasicParse (self):
        xmlt = self.base_xmlt
        xmld = xmlt.encode('utf-8')
        self.assertTrue(isinstance(xmlt, types_.TextType))
        self.assertTrue(isinstance(xmld, types_.DataType))
        pyxb_123._SetXMLStyle(pyxb_123.XMLStyle_saxer)
        instance = CreateFromDocument(xmlt)
        self.assertEqual(instance.e, self.strt)
        instance = CreateFromDocument(xmld)
        self.assertEqual(instance.e, self.strt)
        pyxb_123._SetXMLStyle(pyxb_123.XMLStyle_minidom)
        instance = CreateFromDocument(xmlt)
        self.assertEqual(instance.e, self.strt)
        instance = CreateFromDocument(xmld)
        self.assertEqual(instance.e, self.strt)
        # saxdom can handle Unicode representation
        pyxb_123._SetXMLStyle(pyxb_123.XMLStyle_saxdom)
        instance = CreateFromDocument(xmlt)
        self.assertEqual(instance.e, self.strt)
        instance = CreateFromDocument(xmld)
        self.assertEqual(instance.e, self.strt)

    def testDeclaredParse (self):
        xmlt = self.declared_xmlt
        xmld = xmlt.encode('utf-8')
        self.assertTrue(isinstance(xmlt, types_.TextType))
        self.assertTrue(isinstance(xmld, types_.DataType))
        pyxb_123._SetXMLStyle(pyxb_123.XMLStyle_saxer)
        instance = CreateFromDocument(xmlt)
        self.assertEqual(instance.e, self.strt)
        instance = CreateFromDocument(xmld)
        self.assertEqual(instance.e, self.strt)
        pyxb_123._SetXMLStyle(pyxb_123.XMLStyle_minidom)
        instance = CreateFromDocument(xmlt)
        self.assertEqual(instance.e, self.strt)
        instance = CreateFromDocument(xmld)
        self.assertEqual(instance.e, self.strt)
        # saxdom can handle Unicode representation
        pyxb_123._SetXMLStyle(pyxb_123.XMLStyle_saxdom)
        instance = CreateFromDocument(xmlt)
        self.assertEqual(instance.e, self.strt)
        instance = CreateFromDocument(xmld)
        self.assertEqual(instance.e, self.strt)

    def testElementEncode (self):
        instance = bar()
        instance.e = self.strt
        self.assertEqual(instance.e, self.strt)

    def testAttributeEncode (self):
        instance = bar()
        instance.a = self.strt
        self.assertEqual(instance.a, self.strt)

    def testuEncode (self):
        instance = foo(self.strt)
        self.assertEqual(instance, self.strt)

if __name__ == '__main__':
    unittest.main()
