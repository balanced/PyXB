...
class ExpandedName (object):
    ...
    def __getattr__ (self, name):
        ...
        if self.namespace() is None:
            return lambda: None
        # NOTE: This will raise pyxb_123.NamespaceError if the category does not exist.
    ...
    def adoptName (self, name):
        ...
        L{pyxb_123.binding.basis.element.elementForName}.
        """
    ...
    def __init__ (self, *args, **kw):
        ...
        if 0 == len(args):
            raise pyxb_123.LogicError('Too few arguments to ExpandedName constructor')
        if 2 < len(args):
            raise pyxb_123.LogicError('Too many arguments to ExpandedName constructor')
        if 2 == len(args):
            ...
            if isinstance(ln, basestring):
                ...
                if not(ln.nodeType in (xml.dom.Node.ELEMENT_NODE, xml.dom.Node.ATTRIBUTE_NODE)):
                    raise pyxb_123.LogicError('Cannot create expanded name from non-element DOM node %s' % (ln.nodeType,))
                ns = ln.namespaceURI
                ln = ln.localName
            else:
                raise pyxb_123.LogicError('Unrecognized argument type %s' % (type(ln),))
        ...
        if (ns is not None) and not isinstance(ns, Namespace):
            raise pyxb_123.LogicError('ExpandedName must include a valid (perhaps absent) namespace, or None.')
        self.__namespace = ns
    ...
    def __eq__ (self, other):
        if other is None:
            return False
        return 0 == pyxb_123.utils.utility.IteratedCompareMixed(self.__uriTuple, self.__otherForCompare(other))
...
class _NamespaceCategory_mixin (pyxb_123.cscRoot):
    ...
    def categoryMap (self, category):
        ...
        try:
            ...
            raise pyxb_123.NamespaceError(self, '%s has no category %s' % (self, category))
    ...
    def addCategoryObject (self, category, local_name, named_object):
        ...
        Raises pyxb_123.NamespaceUniquenessError if an object with the same name
        ...
        if (old_object is not None) and (old_object != named_object):
            raise pyxb_123.NamespaceUniquenessError(self, '%s: name %s used for multiple values in %s' % (self, local_name, category))
        ...
        for category in category_map.iterkeys():
            ...
            for (local_name, component) in new_map.iteritems():
                ...
                if existing_component is None:
                    ...
                    raise pyxb_123.NamespaceError(self, 'Load attempted to override %s %s in %s' % (category, local_name, self.uri()))
        self.__defineCategoryAccessors()
    ...
    def _associateOrigins (self, module_record):
        """Add links from L{pyxb_123.namespace.archive._ObjectOrigin} instances.
...
class _ComponentDependency_mixin (pyxb_123.utils.utility.PrivateTransient_mixin, pyxb_123.cscRoot):
    ...
    def bindingRequires (self, reset=False, include_lax=False):
        ...
        returned.
        @rtype: C{set(L{pyxb_123.xmlschema.structures._SchemaComponent_mixin})}
        """
        if reset or (self.__bindingRequires is None):
            if isinstance(self, resolution._Resolvable_mixin) and not (self.isResolved()):
                raise pyxb_123.LogicError('Unresolved %s in %s: %s' % (self.__class__.__name__, self._namespaceContext().targetNamespace(), self.name()))
    ...
    def _bindingRequires_vx (self, include_lax):
        ...
        """
        raise pyxb_123.LogicError('%s does not implement _bindingRequires_vx' % (type(self),))

class _NamespaceComponentAssociation_mixin (pyxb_123.cscRoot):
    ...
    def addSchema (self, schema):
        for sr in self.__origins:
            if isinstance(sr, archive._SchemaOrigin) and sr.match(schema=schema):
                ...
                raise pyxb_123.SchemaUniquenessError(self, schema.location(), sr.schema())
...
from pyxb_123.namespace import archive
from pyxb_123.namespace.utility import NamespaceInstance
from pyxb_123.namespace.utility import NamespaceForURI
from pyxb_123.namespace.utility import CreateAbsentNamespace
from pyxb_123.namespace.utility import AvailableNamespaces
from pyxb_123.namespace import resolution
...
class Namespace (_NamespaceCategory_mixin, resolution._NamespaceResolution_mixin, _NamespaceComponentAssociation_mixin, archive._NamespaceArchivable_mixin):
    ...
    L{Schema<pyxb_123.xmlschema.structures.Schema>} component defines a category
    ...
    def __getnewargs__ (self):
        ...
        if self.uri() is None:
            raise pyxb_123.LogicError('Illegal to serialize absent namespaces')
        return (self.uri(),)
    ...
    def __init__ (self, uri,
        ...
        if not is_builtin_namespace:
            if bound_prefix is not None:
                raise pyxb_123.LogicError('Only permanent Namespaces may have bound prefixes')
    ...
    def setPrefix (self, prefix):
        if self.__boundPrefix is not None:
            if self.__boundPrefix == prefix:
                return self
            raise pyxb_123.NamespaceError(self, 'Cannot change the prefix of a bound namespace')
    ...
    def builtinNamespaceRepresentation (self):
        ...
        return 'pyxb_123.namespace.%s' % (self.__builtinNamespaceVariable,)

    def builtinModulePath (self):
        from pyxb_123.namespace import builtin
        if not self.__builtinModulePath:
            raise pyxb_123.LogicError('Namespace has no built-in module: %s' % (self,))
        ...
        for mr in self.moduleRecords():
            if mr.isLoadable():
                if mr.isPublic():
                    ...
                    except pyxb_123.NamespaceArchiveError:
    ...
    def validateComponentModel (self, structures_module=None):
        ...
        if not self.__didValidation:
            ...
            if structures_module is None:
                import pyxb_123.xmlschema.structures as structures_module
    ...
    def __identifyNamespace (self, nsval):
        ...
        if isinstance(nsval, Namespace):
            return nsval
        raise pyxb_123.LogicError('Cannot identify namespace from %s' % (nsval,))
...
from pyxb_123.namespace.builtin import XMLSchema_instance
from pyxb_123.namespace.builtin import XMLNamespaces
from pyxb_123.namespace.builtin import XMLSchema
from pyxb_123.namespace.builtin import XHTML
from pyxb_123.namespace.builtin import XML
from pyxb_123.namespace.builtin import XMLSchema_hfp
from pyxb_123.namespace.builtin import BuiltInObjectUID
